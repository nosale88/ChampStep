-- 기존 테이블 구조에 맞춰서 누락된 테이블들 생성
-- 실제 존재하는 컬럼만 사용하여 정책 작성

-- =============================================
-- 1. 기존 competitions 테이블에 필요한 컬럼 추가
-- =============================================

-- competitions 테이블에 누락된 컬럼들 추가 (있으면 무시)
...
CREATE POLICY "Update own participation" ON competition_participants
  FOR UPDATE USING (
    EXISTS (
      ...
      WHERE dancers.id = competition_participants.dancer_id 
      AND dancers.user_id = auth.uid()
    )
  );
...
-- 참석자 정보는 모든 사용자가 볼 수 있음
CREATE POLICY "View schedule attendees" ON schedule_attendees
  FOR SELECT USING (true);

-- 인증된 사용자만 참석 정보 수정 가능
CREATE POLICY "Update attendance" ON schedule_attendees
...
CREATE POLICY "View messages" ON messages
  FOR SELECT USING (
    ...
    (target_type = 'dancer' AND EXISTS (
      SELECT 1 FROM dancers 
      WHERE dancers.id = messages.target_id 
      AND dancers.user_id = auth.uid()
    )) OR
    (target_type = 'crew' AND EXISTS (
      ...
      WHERE dancers.user_id = auth.uid() 
      AND dancers.crew = (
        SELECT name FROM crews WHERE crews.id = messages.target_id
      )
    ))
  );
