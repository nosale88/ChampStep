-- 기존 테이블 구조에 맞춰서 추가 테이블들만 생성
-- 기존 competitions, dancers, crews 테이블은 그대로 유지

-- =============================================
-- 1. 크루 스케줄 테이블 (기존 테이블 없음)
-- =============================================

-- 크루 스케줄 테이블
CREATE TABLE IF NOT EXISTS crew_schedules (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  crew_id TEXT NOT NULL, -- crews.id는 TEXT 타입
...
CREATE TABLE IF NOT EXISTS messages (
  ...
  target_id TEXT NOT NULL, -- dancers.id와 crews.id는 TEXT 타입
  is_public BOOLEAN DEFAULT true,
...
-- =============================================
-- 3. 대회 관련 추가 테이블 (기존 competition_participants 수정)
-- =============================================

-- 기존 competition_participants 테이블에 컬럼 추가 (있으면 무시)
ALTER TABLE competition_participants 
ADD COLUMN IF NOT EXISTS team_name VARCHAR(255),
ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'registered' CHECK (status IN ('registered', 'confirmed', 'participated', 'withdrawn'));
