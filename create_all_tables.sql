-- ChampStep 데이터베이스 테이블 생성 스크립트
-- 실행 순서: 1. 기본 테이블 → 2. 외래키 테이블 → 3. 인덱스 및 정책

-- =============================================
-- 1. 기본 테이블 생성 (외래키 의존성 없음)
-- =============================================
...
-- 모든 사용자가 대회 조회 가능
CREATE POLICY "View competitions" ON competitions
  FOR SELECT USING (true);

-- 인증된 사용자만 대회 생성 가능
CREATE POLICY "Create competitions" ON competitions
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- 대회 생성자만 수정 가능
CREATE POLICY "Update competitions" ON competitions
  FOR UPDATE USING (created_by = auth.uid());

-- 대회 생성자만 삭제 가능
CREATE POLICY "Delete competitions" ON competitions
